...
async function requireChannelAccess(
  ...
  if (server.ownerId !== userId) {
    const membership = await ctx.db
      ...
      .withIndex('by_server_id_user_id', (q) => q.eq('serverId', channel.serverId).eq('userId', userId))
      .unique()
...
export const sendMessage = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const { channelId } = await requireChannelAccess(ctx, args.channelId, authorId)

    const content = args.content.trim()
    if (content.length < 1 || content.length > 2000) {
      throw new ConvexError('Message content must be between 1 and 2000 characters.')
    }
...
export const listMessages = query({
  ...
  handler: async (ctx, args) => {
    ...
    const messages = await ctx.db
      .query('messages')
      .withIndex('by_channel_id_created_at', (q) => q.eq('channelId', channelId))
      .collect()

    const authors = await Promise.all(messages.map((message) => ctx.db.get(message.authorId)))
